 ...
 * @private
 */
function getRefSchema (refVal, refType, parent, options, state, node) {
  ...
  if (refType && loader) {
    ...
    if (!loaderValue) {
      loaderValue = loader(refVal, options, node)
      if (loaderValue) {
  ...
  return traverse(schema).forEach(function (node) {
    if (!_.isNull(node) && !_.isUndefined(null) && typeof node.$ref === 'string') {
      ...
      if (!addOk) {
        ...
        let newValue = getRefSchema(refVal, refType, schema, options, state, node)
        state.history.pop()
